...
ax1.set_ylabel("Pages",color='slateblue')
ax1.set_title("")
ax1.text(0.1*maxdayssince, 0.9*max(pages), r"%s pages"   % (lastpages), color='turquoise')
ax1.text(0.1*maxdayssince, 0.8*max(pages), r"%s words"   % (lastwords), color='slateblue')
...
ax1.fill_between(dayssince, 0, pages, facecolor='turquoise', interpolate=True)

ax2 = ax1.twinx()
ax2.set_ylabel('Words',color='turquoise')
ax2.yaxis.set_label_coords(1.1, 0.4)
# print dayssince, words
ax2.plot(dayssince, words, "-",color='slateblue',linewidth=2)
