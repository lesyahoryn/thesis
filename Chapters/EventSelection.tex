...
Quality cuts specifically designed for this analysis are introduced to eliminate fake electrons reconstructed from the incorrect association of a track from \ac{LRT} and a calorimeter cluster. The most important of these cuts is $\dpt = \frac{p_{T, \text{track}} - p_{T, e}}{p_{T, e}}$, which measures the degree of consistency between the \pt of the electron, which is dominated by the calorimeter measurement, and the \pt of the track. Fake tracks tend to be low \pt, resulting in fake electrons with $\dpt < -0.5$, or tracks with less than half of the \pt of the electron.

Additionally, quality requirements are imposed on the \ac{ID} tracks. They are required to have $\chiID < 2$ \footnote{$\chi^{2}$ in this thesis implies $\chi^{2}$ divided by the number of degrees of freedom in the fit} and no more than 1 missing hit after the first hit. These cuts remove poor hit combinations, short tracks, or tracks missing many measurements. Additionally, signal electrons are required to be isolated and pass the \texttt{FCTight} isolation definition described in \autoref{sec:isolation}
